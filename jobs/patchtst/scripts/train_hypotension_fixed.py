"""Leakage-safe downstream hypotension probe for a pretrained PatchTST checkpoint."""
...
class FrozenPatchTSTProbe(PatchTFTSingleOutcomeLightning):
    """Keep the PatchTST encoder in evaluation mode during linear probing."""
...
def main() -> None:
    config, config_path = load_config("configs/train_hypotension_fixed.yaml")
    ...
    num_heads = encoder.model.layers[0].self_attn.num_heads
    print(f"Loaded PatchTST encoder: d_model={d_model}, num_heads={num_heads}")
    ...
    head = AttentiveClassifier(
        embed_dim=d_model,
        num_heads=int(head_config.get("num_heads", num_heads)),
