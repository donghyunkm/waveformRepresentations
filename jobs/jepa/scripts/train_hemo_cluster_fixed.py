"""Attentive probe for hemodynamic cluster classification using a frozen native JEPA encoder.
...
class MulticlassProbe(PatchTFTSingleOutcomeLightning):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if y.dtype in (torch.float32, torch.float16, torch.bfloat16) and y.dim() == 2:
            # Mixup soft targets: manual cross-entropy
...
class HemoClusterDataset(Dataset):
    ...
    Only includes windows that have valid (>= 0) hemo cluster labels.
    """
    ...
    def __getitem__(self, idx):
        ...
        x, _ = self.base_dataset[real_idx]  # Discard original label
...
def load_hemo_clusters(config: dict) -> np.ndarray:
    """Load window-level hemodynamic cluster labels aligned to the test sample cache.

    These are aligned to the test split only (127,811 windows). For train/val we
    need to compute alignment separately or filter to patients with labels.
    """
    path = Path(config["paths"]["hemo_clusters_path"])
    if not path.exists():
        raise FileNotFoundError(f"Hemo clusters file not found: {path}")
    data = np.load(path, allow_pickle=True)
    return data["hemo_clusters"], data["patient_ids"]
...
def main() -> None:
    ...
    # For the test set, try to use pre-computed hemo clusters if available
    ...
    for split in ("train", "val", "test"):
        ...
        if cache_path.is_file():
            ...
            # The pre-computed clusters are aligned to test set
    ...
    # Build wrapped datasets (only valid hemo windows)
    datasets = {}
    for split in ("train", "val", "test"):
        outcomes, samples = split_inputs[split]
