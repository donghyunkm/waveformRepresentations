...
from physiojepa.heads import AttentiveClassifierNoMelt
...
config_path = 'train_hypotension_ecgjepa.yaml'
...
encoder_model = ECGJEPALightning.load_from_checkpoint(pretrained_encoder_path, 
                                                      map_location='cpu')

d_model = encoder_model.encoder.embed_dim 
num_heads = encoder_model.encoder.encoder_blocks.blocks[0].attn.num_heads
...
lp_model = AttentiveClassifierNoMelt(**lp_head)
...
if __name__ == "__main__":
    ...
    if config['training']['use_lr_finder']:
        tuner = Tuner(trainer)
        lr_finder = tuner.lr_find(patchmeupe2e_model, train_dataloaders=train_loader, update_attr=False, attr_name="max_lr")
        new_lr = lr_finder.suggestion()
        patchmeupe2e_model.scheduler_kwargs['max_lr'] = new_lr
        print(f"Using max lr: {new_lr}")
    
    trainer.fit(model=patchmeupe2e_model, train_dataloaders=train_loader, val_dataloaders=val_loader)
    ...
    torch.save(tensor_dict, os.path.join(models_dir, sub_dir, f'{config["dataset"]["y_outcome"]}-{config["run_config"]["name"]}-{"|".join(channels)}-predictions.pt'))
